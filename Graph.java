...
public class Graph {
	...
	private int countNodes; //contador de vertices/nos
    private int countEdges; //contador de arestas
    private int [][] adjMatrix; // matriz
	...
	public void addEdge(int source, int sink, int weight) { //adc arestas= origem, destino, peso
	    if(source < 0 || source > this.countNodes -1
		   || sink < 0 || sink > this.countNodes -1
		   || weight <= 0) {
			System.out.println("\n---------------------------------------------");
	        System.err.println("Invalid adge: " + " " + source + " " 
			+ sink + " " + weight);
			return;
	    }
	...
	public Graph complement() {
		Graph g2 = new Graph(this.countNodes); //instanciando nova matriz
		for(int i = 0; i < this.adjMatrix.length; ++i) {
			for (int j = 0; j < this.adjMatrix[i].length; ++j) {
